...
@Component({
  selector: 'app-facility-manag',
  standalone: true,
  imports: [BusinessInsustrySubmenu, CommonModule, RouterLink],
  templateUrl: './facility-manag.html',
  styleUrl: './facility-manag.css',
})
export class FacilityManag implements OnInit {
  public module: any;
  public next: any;
  ...
  ngOnInit() {
    ...
    this.next = this.$rootScope.allModule?.[2];

    // Prevent SSR crash if module is unavailable
    if (!this.module) {
      console.error('FacilityManag: module not found at index:', this.var);
      return;
    }
    ...
    // IMPORTANT:
    // selected can be undefined during SSR
