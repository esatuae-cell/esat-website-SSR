...
export class AssetTraining implements OnInit {
  ...
  constructor(
    ...
    this.titleService.setTitle('Asset Management Company in Dubai | ESAT ERP - Asset Management');
  ...
  ngOnInit(): void {
    ...
    if (this.$rootScope?.webData && pagelink && this.$rootScope.webData[pagelink]) {
      this.dataValue = this.$rootScope.webData[pagelink];
    } else if (pagelink) {
      this.http.get(this.$rootScope.httpLink + pagelink).subscribe({
